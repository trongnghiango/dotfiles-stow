#!/usr/bin/env bash
# ==============================================================================
# FEDORA LINUX SETUP DRIVER (setup-drivers/fedora.sh)
# Mô tả: Driver cài đặt gói cho Fedora / RHEL qua DNF (dnf install)
...
fedora_setup_pkgs() {
  ...
  local progs_file="$dotfiles_root/pkgs/fedora.csv"

  if [ ! -f "$progs_file" ]; then
    log_error "Không tìm thấy file danh sách gói Fedora: $progs_file"
    return 1
  fi

  if ! command -v dnf &>/dev/null; then
    log_warn "Hệ thống không dùng dnf — bỏ qua bước cài đặt Fedora."
    return 0
  fi

  if [ -z "$target" ]; then
    if [ -t 0 ]; then
      echo -e "\033[1mChọn profile gói cài đặt cho Fedora:\033[0m"
      ...
      echo -e "  \033[0;32m3) media\033[0m  Nghe nhạc, xem phim (MPV, PipeWire...)"
  ...
  log_step "Đang đọc danh mục gói Fedora ($target) từ: $progs_file"
  ...
  log_step "Tiến hành cài đặt qua dnf install..."
  sudo dnf install -y "${pkgs_to_install[@]}"
  log_success "Hoàn tất cài đặt gói Fedora!"
}
